function codecheckToolbox(rootDir)
    ...
    if isempty(filesToCheck)
        error("%s:codeissues",string("test_workflow7"),"No files to check.")
    else
        ...
        if issueCount ~= 0
            ...
            error("%s:codeissues",string("test_workflow7"),"%s Toolbox requires all code check issues be resolved.",string("test_workflow7"))
        end
    end
end
